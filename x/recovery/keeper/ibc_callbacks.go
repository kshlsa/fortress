...
//  - sends over all Fortress native tokens
...
func (k Keeper) OnRecvPacket(
	...
	// a sender's own account on Fortress (sender == recipient)
	...
	if account != nil && fortress.IsSupportedKey(account.GetPubKey()) {
		return ack
	}
	...
	k.bankKeeper.IterateAccountBalances(ctx, recipient, func(coin sdk.Coin) (stop bool) {
		...
		err = k.transferKeeper.SendTransfer(
			...
			recipient,                 // recipient is the address in the Fortress chain
...
// the IBC denomination, i.e port and channel on Fortress for the voucher. It
// returns an error if:
