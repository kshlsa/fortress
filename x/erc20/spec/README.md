...
This document specifies the internal `x/erc20` module of the Torque Hub.

The `x/erc20` module enables the Torque Hub to support a trustless, on-chain bidirectional internal conversion of tokens between Torque' EVM and Cosmos runtimes, specifically the `x/evm` and `x/bank` modules. This allows token holders on Torque to instantaneously convert their native Cosmos `sdk.Coins` (in this document referred to as "Coin(s)") to ERC-20 (aka "Token(s)") and vice versa, while retaining fungibility with the original asset on the issuing environment/runtime (EVM or Cosmos) and preserving ownership of the ERC-20 contract.
...
With the `x/erc20` users on Torque can
...
- transfer existing tokens on Ethereum and other EVM-based chains to Torque to take advantage of application-specific chains in the Cosmos ecosystem
