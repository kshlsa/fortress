...
`fortressd` is the all-in-one command-line interface. It supports wallet management, queries and transaction operations {synopsis}
...
After you have obtained the latest `fortressd` binary, run:
...
::: tip
You can also enable auto-completion with the `fortressd completion` command. For example, at the start of a bash session, run `. <(fortressd completion)`, and all `fortressd` subcommands will be auto-completed.
:::

### Config and data directory

By default, your config and data are stored in the folder located at the `~/.fortressd` directory.
...
To specify the `fortressd` config and data storage directory; you can update it using the global flag `--home <directory>`

### Configuring the Node

The Cosmos SDK automatically generates two configuration files inside `~/.fortressd/config`:
...
We can view the default client config setting by using `fortressd config` command:
...
For example, the chain identifier can be changed to `fortress_9000-4` from a blank name by using:
...
After the necessary changes are made in the `client.toml`, then save. For example, if we directly change the chain-id from `fortress_{{ $themeConfig.project.testnet_chain_id }}-2` to `fortresstest_9000-1`, and output to number, it would change instantly as shown below.

```bash
fortressd config
{
 "chain-id": "fortresstest_9000-1",
 "keyring-backend": "os",
 "output": "number",
 "node": "tcp://localhost:26657",
 "broadcast-mode": "sync"
...
A list of commonly used flags of `fortressd` is listed below:

| Option              | Description                   | Type                                             | Default Value |
| ------------------- | ----------------------------- | ------------------------------------------------ | ------------- |
| `--chain-id`        | Full Chain ID                 | `string`                                         | `""`          |
| `--home`            | Directory for config and data | `string`                                         | `~/.fortressd`   |
| `--keyring-backend` | Select keyring's backend      | `{"os"|"file"|"kwallet"|"pass"|"test"|"memory"}` | `"os"`        |
| `--output`          | Output format                 | `string`                                         | "text"        |

## Command list

A list of commonly used `fortressd` commands. You can obtain the full list by using the `fortressd -h` command.
