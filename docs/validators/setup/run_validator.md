...
```bash
fortressd tx staking edit-validator
  --moniker="choose a moniker" \
...
```bash
fortressd query staking validator <account_cosmos>
```
...
```bash
fortressd query slashing signing-info <validator-pubkey>\
  --chain-id=<chain_id>
```
...
```bash
fortressd query tendermint-validator-set | grep "$(fortressd tendermint show-address)"
```

You should now see your validator in one of Fortress explorers. You are looking for the `bech32` encoded `address` in the `~/.fortressd/config/priv_validator.json` file.
...
you want your node to shutdown or by passing the `--halt-height` flag to `fortressd`.
...
If you got jailed for downtime, you can get your voting power back to your validator. First, if `fortressd` is not running, start it up again:
...
The default number of files Linux can open (per-process) is `1024`. `fortressd` is known to open more than `1024` files. This causes the process to crash. A quick fix is to run `ulimit -n 4096` (increase the number of open files allowed) and then restart the process with `fortressd start`. If you are using `systemd` or another process manager to launch `fortressd` this may require some configuration at that level. A sample `systemd` file to fix this issue is below:

```toml
# /etc/systemd/system/fortressd.service
[Unit]
...
ExecStart=/home/ubuntu/go/bin/fortressd start
Restart=on-failure
RestartSec=3
LimitNOFILE=4096
