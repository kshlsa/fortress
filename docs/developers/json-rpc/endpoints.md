...
Check the JSON-RPC methods supported on Fortress. {synopsis}
...
```json
 {"jsonrpc":"2.0","id":1,"result":"Fortress/0.1.3+/linux/go1.17"}
```
...
Make sure your `fortressd start` call is not using the flag `minimum-gas-prices` because this value will be used instead of the one set on the configuration file.
