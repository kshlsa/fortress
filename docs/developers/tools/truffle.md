...
Learn how to deploy a simple Solidity-based smart contract to Fortress using the Truffle environment {synopsis}
...
This will allow your contract to connect to your Fortress local node.
...
You should see incoming deployment logs in the Fortress daemon Terminal tab for each transaction (one to deploy `Migrations.sol` and the other to deploy `Counter.sol`).
...
Now, you can run the Truffle tests using the Fortress node using the `test` command:
