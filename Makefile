...
SIMAPP = ./app
HTTPS_GIT := https://github.com/olegtropinin/torque.git
DOCKER := $(shell which docker)
DOCKER_BUF := $(DOCKER) run --rm -v $(CURDIR):/workspace --workdir /workspace bufbuild/buf
NAMESPACE := olegtropinin
PROJECT := torque
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/olegtropinin/torque/types"
	godoc -http=:6060
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/docs/statik/statik.go" -not -name '*.pb.go' | xargs goimports -w -local github.com/olegtropinin/torque
.PHONY: format
...
PACKAGE_NAME:=github.com/olegtropinin/torque
